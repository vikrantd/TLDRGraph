"""
Graph Loader and Multi-Layer Builder for CodeChakra.
...
#: Schema version of .codechakra/graph.json
SNAPSHOT_SCHEMA_VERSION = 1

#: Filename of the persisted CodeChakra graph snapshot (inside .codechakra/).
...
class GraphLoader:
    def __init__(self, root_dir: str = ".", embeddings: Optional[str] = None):
        ...
        *embeddings* selects the retrieval backend policy (``off`` / ``auto`` /
        ``on``). ``None`` -- the default -- defers to ``CODECHAKRA_EMBEDDINGS``,
        ...
        self.hash_gate = HashGate(os.path.join(self.root_dir, ".codechakra/codechakra.db"))
        self.vector_store = LocalVectorStore(
            os.path.join(self.root_dir, ".codechakra/vector_index.json"),
    ...
    def snapshot_path(self) -> str:
        return os.path.join(self.root_dir, ".codechakra", SNAPSHOT_FILENAME)

    def save_graph(self) -> str:
        """
        Persists the full node + edge graph (including cross-layer bridges) to
        .codechakra/graph.json. Returns the written path.
        """
        ...
        for node_id, data in self.graph.nodes(data=True):
            nodes.append({
                ...
                # targets are matched against -- see codechakra.labels.
        ...
        snapshot = {
            "codechakra_version": __version__,
    ...
    def load_graph_snapshot(self) -> Optional[Dict[str, Any]]:
        """Reads .codechakra/graph.json. Returns None if absent or corrupt."""
        ...
        if os.path.exists(graph_json_path):
            ...
            for node in raw_nodes:
                ...
                if usable_cache:
                    ...
                    # the real source recorded in .codechakra/graph.json.
    ...
    def export_yaml(self, output_dir: str = None) -> str:
        ...
        if output_dir is None:
            output_dir = os.path.join(self.root_dir, ".codechakra")
