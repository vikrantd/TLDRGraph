"""
Agent Installer for CodeChakra.
...
enrichment loop, plus the agent contract they all point at:
    ...
    <root>/.codechakra/AGENT_CONTRACT.md      -- the request/response schema
    <root>/.claude/skills/codechakra/SKILL.md -- Claude Code skill
    <root>/CLAUDE.md                          -- delimited CodeChakra section
    <root>/.cursor/rules/codechakra.mdc       -- Cursor project rule
    <root>/.agents/rules/codechakra.md        -- Antigravity rule
    <root>/.agents/workflows/codechakra.md    -- Antigravity workflow

Every write is idempotent: unchanged files are left alone, and CLAUDE.md is
*never* clobbered -- only the region between the CodeChakra markers is replaced.
"""
...
CLAUDE_MD_BEGIN = "<!-- BEGIN CODECHAKRA -->"
CLAUDE_MD_END = "<!-- END CODECHAKRA -->"

#: Where the contract is installed inside a target repository.
CONTRACT_REL_PATH = os.path.join(".codechakra", "AGENT_CONTRACT.md")
...
removing anything. CodeChakra has no delete capability."""
...
def make_agent_contract_fallback(registry: Optional[LayerRegistry] = None) -> str:
    ...
    return f"""# CodeChakra Agent Contract

**Audience: the coding agent with this repository open.**

CodeChakra maps this codebase into {count} architectural layers:
...
`queue-enrichment` records progress in `.codechakra/enrichment_cursor.json`, so running it
...
description: >-
  ...
  endpoint, or when asked to enrich / describe un-enriched CodeChakra nodes.
---

# CodeChakra: {count}-Layer Code Flow & Semantic Navigation
...
```bash
codechakra query "<feature in plain English>"    # semantic search + end-to-end flow
codechakra trace "<Source>" "<Target>"           # exact path between two symbols
codechakra layers                                # node counts per layer
```

`query`, `trace`, `layers` and `dead-code` are read-only: they never fire enrichment.
`.codechakra/layers.yaml` and `.codechakra/flows.yaml` hold the exported context.
...
Response format (`.codechakra/enrichment_response.json`), a bare JSON array:
...
def make_claude_md_section(registry: Optional[LayerRegistry] = None) -> str:
    ...
    return f"""## CodeChakra ({count}-layer code flow engine)

This repo is mapped by CodeChakra into {count} architectural layers ({names}). Snapshot: `.codechakra/graph.json`, summaries:
`.codechakra/layers.yaml`.

**Before planning or implementing a feature**, trace it rather than grepping:

```bash
codechakra query "<feature in plain English>"
codechakra trace "<Source>" "<Target>"
```
...
`codechakra dead-code` lists **review candidates, not confirmed dead code**; `unreviewed`
...
def make_cursor_rule(registry: Optional[LayerRegistry] = None) -> str:
    ...
    count = len(registry.ordered()) - 1 if registry else 6
    return f"""---
description: CodeChakra {count}-layer code flow, semantic navigation, and agent enrichment loop
globs: ["**/*"]
alwaysApply: true
---

# CodeChakra Multi-Layer Code Flow Rules

{layers_prose}

## Navigate before you edit

- Run `codechakra query "<feature>"` or `codechakra trace "<Source>" "<Target>"` to get
  the real end-to-end path across layers before planning a change.
- Read `.codechakra/layers.yaml` and `.codechakra/flows.yaml` for architectural context.
...
def make_antigravity_rule(registry: Optional[LayerRegistry] = None) -> str:
    ...
    count = len(registry.ordered()) - 1 if registry else 6
    return f"""---
description: CodeChakra {count}-Layer Code Flow & Semantic Navigation Engine
globs: **/*
---

# CodeChakra Multi-Layer Code Flow Rules

This project uses CodeChakra to map full-stack code into {count} layers:

{layers_prose}

## Rules for Antigravity

- Before planning or implementing any feature, run `codechakra query "<feature>"` or
  `codechakra trace "<Source>" "<Target>"` to trace exact end-to-end execution paths.
- Read `.codechakra/layers.yaml` and `.codechakra/flows.yaml` for architectural context.
...
# Workflow: CodeChakra Flow Engine

1. `codechakra scan .` -- refresh AST, layers, and the local vector index.
2. `codechakra query "<feature>"` -- inspect the full {count}-layer execution path.
3. `codechakra layers` -- architectural layer summary.
4. Enrichment loop, highest-value nodes first:

{_LOOP}

5. `codechakra dead-code` -- list review candidates (never a delete list).
...
def upsert_delimited_section(existing: Optional[str], section_body: str) -> str:
    """
    Returns ``existing`` with the CodeChakra-delimited region replaced by
...
def install_agent_rules(root_dir: str = ".") -> Dict[str, str]:
    """
    Installs CodeChakra rules for Claude Code, Cursor and Antigravity, plus the shared
    agent contract.
    """
    ...
    skill_path = os.path.join(root, ".claude", "skills", "codechakra", "SKILL.md")
...
def gitignore_warnings(root_dir: str = ".") -> List[str]:
    ...
    watched = {".agents": ".agents/", ".claude": ".claude/", ".cursor": ".cursor/",
               ".codechakra": ".codechakra/", "CLAUDE.md": "CLAUDE.md"}
    warnings: List[str] = []
