"""
Multi-Layer Compound Hierarchy Builder for CodeChakra.

Emits a **3-tier** hierarchical graph (schema ``codechakra/hierarchy@2``):
...
identity (:func:`codechakra.extractors.collect_endpoints`), living in the API
...
as graphify emitted it -- see :mod:`codechakra.labels` for why that separation is
load-bearing.
...
HIERARCHY_SCHEMA = "codechakra/hierarchy@2"
...
def build_multilayer_hierarchy(root_dir: str = ".") -> Dict[str, Any]:
    ...
    graph_json_path = os.path.join(root_dir, ".codechakra", "graph.json")
    ...
    # Owner-qualified labels. `label` is left untouched -- see codechakra.labels.
    ...
    for fpath, nlist in ast_by_file.items():
        ...
        for n in nlist:
            # When the source graph is the CodeChakra snapshot rather than raw
    ...
    #
    # The deterministic seams are skipped here even though the CodeChakra
    ...
    # depend on whether it happened to read graphify-out/ or .codechakra/.
    for e in ast_edges:
