"""
Local Vector Store for TLDRGraph (100% offline, $0 token cost).
...
capability it does not have -- run ``tldrgraph doctor`` to see the truth.
...
Enablement policy
-----------------
Controlled by ``TLDRGRAPH_EMBEDDINGS`` (or the ``embeddings=`` constructor
argument):
...
class LocalVectorStore:
    def __init__(self, index_path: str = ".tldrgraph/vector_index.json",
    ...
    # ------------------------------------------------------------------ #
    # Diagnostics (backs `tldrgraph doctor`)
    # ------------------------------------------------------------------ #
    ...
    def _load(self):
        ...
        if version != INDEX_FORMAT_VERSION:
            # An index from an older TLDRGraph. Do NOT raise and do NOT try to
